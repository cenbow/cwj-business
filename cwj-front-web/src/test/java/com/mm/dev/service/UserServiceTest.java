...
//    	logger.info("mybatis分页：{}",JSONObject.toJSONString(userAll));
	...
	@Test
	public void testFindOrderPaymentUserFilesByOpenId() throws Exception {
		Sort sort = new Sort(Direction.DESC, "createTime");
		Pageable pageable = new PageRequest(0,2, sort);
		System.out.println(JSONObject.toJSONString(userService.queryOrderPaymentUserFilesByOpenId("oIrGq0x10Ii4_F0_FKD8XYhdr7Lk",pageable)));
	}
	...
	public void testSave() {
		...
		user.setOpenId("22222222222");
		user.setUserName("test");
		user.setNickName("test");
		System.out.println(JSONUtil.toJsonPrettyStr(userService.save(user)));
	}
	
	@Test
	public void updateUserBalanceInfo() throws Exception {
		//更新当前用户账户余额，与收入总额
		User user = new User();
		user.setOpenId("oIrGq0x10Ii4_F0_FKD8XYhdr7Lk");
		user.setBalance(new BigDecimal("2.00"));
		user.setTotalIncome(new BigDecimal("2.00"));
		user.setWithdrawBalance(new BigDecimal("2.00"));
