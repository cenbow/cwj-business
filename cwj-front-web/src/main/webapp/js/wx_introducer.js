...
function initData(openId) {
	...
	my.ajaxGet("/userRecommend/userRecommendQrImage?refOpenid="+openId,function(ret,err,status){
		if(ret && ret.status && !my.isNotEmpty(sessionStorage.sceneUrl)){
			sessionStorage.sceneUrl = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket="+ret.data;
			$("#sceneUrl").attr("src",sessionStorage.sceneUrl);
		} else {
			$("#sceneUrl").attr("src",sessionStorage.sceneUrl);
		}
	},true);
