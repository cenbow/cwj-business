...
public class WechatController extends BaseController{
	...
	public ModelAndView oauth2authorize(@RequestParam(value = "code") String authcode, @RequestParam(value = "state") String state,
		...
		try {
			...
			if ("1".equals(state)) {
				...
				imagePath.append(gotoPage);
        		imagePath.append("/");
        		imagePath.append("wx_image_wxpay.html?");
        		imagePath.append("id=");
        		imagePath.append(id);
				gotoPage = imagePath.toString();
			} else if("4".equals(state)) {
				//跳转我的文件列表
				gotoPage += "/index.html?openId="+openId;
			} else if("5".equals(state)) {
				//跳转打赏圈首页
				gotoPage += "/index.html?userOpenId="+openId;
			} else if("6".equals(state)) {
				//跳转上传视频
				gotoPage += "/wx_upload_video.html";
				...
				//跳转我打赏过的照片
				gotoPage += "/index.html?openId="+openId+"&isMyOrder=true";
			}
	...
	@OperationLog(value = "获取微信支付签名并支付")
	public WechatPayConfig getWechatPayConfig(String productId, HttpServletRequest request) throws Exception {
		logger.info("获取微信支付签名开始……productId:" + productId);
		WechatPayConfig config = new WechatPayConfig();
		try {
			String openId = (String)UserSession.getSession(WechatConstant.OPEN_ID);
//			String openId = "o5z7ywOP7qycrtAAxIqDfgMbfcFY";
			logger.info("获取微信支付……openId:" + openId);
			if (!StringUtils.isEmpty(productId) && !StringUtils.isEmpty(openId)) {
				 String amount = "0.01";
				logger.info("amount===" + amount);
				config = wechatService.getWechatPayConfig(productId, openId, amount, request);
			}
		} catch (Exception e) {
	...
	/**
	 * @Description: 微信支付回调
	 * @DateTime:2017年11月7日 下午2:02:02
	 * @return void
	 * @throws
	 */
	@ResponseBody
	@RequestMapping("/payCallBack")
	@OperationLog(value = "微信支付回调")
	public void payCallBack(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if ("SUCCESS".equals(return_code) && "SUCCESS".equals(result_code)) {
				...
				orderPayment.setOrderNo(resultMap.get("out_trade_no"));
				orderPayment.setPaymentTime(DateUtil.date());
				orderPayment.setPaymentMethod(PaymentMethodEnum.getDescription(resultMap.get("trade_type")));
				orderPayment.setPaymentStatus(PaymentStatusEnum.success.getIndex());
				orderPayment.setPaymentBank(WXBankTypeEnum.getDescription(resultMap.get("bank_type")));
				orderPaymentService.updateByOrderNo(orderPayment);
				//根据订单号更新发布人账户余额,更新订单明细结算状态，新增收支明细记录
				orderPaymentService.updateUserBalanceInfoByOrderNo(orderPayment.getOrderNo());
				...
				logger.info("已返回成功结果resultMap={}",resultMap);
			}
		} catch (Exception e) {
			logger.error("微信支付回调通知接口异常" + e);
			try {
				PrintWriter out = response.getWriter();
